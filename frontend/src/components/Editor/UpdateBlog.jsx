...
const UpdateBlog = () => {
    ...
    const saveBlogMutation = useMutation(
        (blogData) => updateBlog(id, blogData), {
        onSuccess: (data) => {
    ...
    const saveContent = async () => {
        ...
        editorRef.current.save().then((content) => {
            ...
            if (image.file || true) {
                // const customName = 'img_'+ (image.url.split('img_')[1]).split('.')[0]
                const parts = blogData.photo.split('/')
                const customName = parts[parts.length - 1].split('.')[0]
